...
#     fi
    ...
    sudo echo -e "blacklist evbug" | sudo tee -a /etc/modprobe.d/disable-evbug.conf &>> $INSTLOG
    #Fix sound problems
    sudo echo -e "blacklist snd_pcsp" | sudo tee -a /etc/modprobe.d/snd_pcsp.conf &>> $INSTLOG
    sudo echo -e "blacklist pcspkr" | sudo tee -a /etc/modprobe.d/nobeep.conf &>> $INSTLOG
